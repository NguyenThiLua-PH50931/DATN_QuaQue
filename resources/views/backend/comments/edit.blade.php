                            ...
                            @if ($comment)
                                <div class="mb-4">
                                    ...
                                    <p><strong>Người dùng:</strong>
                                        {{ optional($comment->user)->name ?? 'Người dùng đã xóa' }}</p>
                                    <p><strong>Sản phẩm:</strong>
                                        {{ optional($comment->product)->name ?? 'Sản phẩm đã xóa' }}</p>
                                ...
                                <form method="POST" action="{{ route('admin.comments.update', $comment->id) }}">
                                    ...
                                    <a href="{{ route('admin.comments.index') }}" class="btn btn-secondary btn-sm">Quay
                                        lại</a>
                                </form>

                                <!-- Nút riêng để trả lời -->
                                <a href="{{ route('admin.comments.reply', $comment->id) }}"
                                    class="btn btn-info btn-sm mt-3">Trả lời bình luận</a>
                                ...
                                @if ($comment->replies->count() > 0)
                                    ...
                                    @foreach ($comment->replies as $reply)
                                        <div class="card mb-2">
                                            <div class="card-body">
                                                {{-- Hiển thị nội dung an toàn, giữ newline --}}
                                                <p>{!! nl2br(e($reply->reply)) !!}</p>

                                                {{-- Lấy tên tác giả an toàn: ưu tiên admin, sau đó user, fallback --}}
                                                @php
                                                    $authorName =
