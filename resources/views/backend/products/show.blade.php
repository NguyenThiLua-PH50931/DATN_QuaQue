...
@section('content')
                ...
                <div class="tab-pane fade" id="variants" role="tabpanel" aria-labelledby="variants-tab">
                    <form id="bulk-delete-variants-form" method="POST"
                        ...
                        <div class="product-table-wrapper">
                                ...
                                <tbody>
                                    @foreach ($product->variants as $variant)
                                        ...
                                        <input type="checkbox" class="variant-checkbox row-checkbox" name="variant_ids[]" value="{{ $variant->id }}">
                                            ...
                                            <td>
                                                <a href="{{ route('admin.products.variant.show', $variant->id) }}"
                                                    class="fw-bold text-primary" style="font-size:16px;">
                                                    {{ $variant->name }}
                                                </a>
                                            </td>
                                            {{-- <td>
                            ...
                            <button type="button" id="delete-selected-variants"
                                class="btn bulk-delete-btn btn-sm mt-2 d-inline-flex align-items-center gap-2"
                                data-bs-toggle="modal" data-bs-target="#deleteBulkVariantModal" disabled>
    ...
    <div class="modal fade" id="changeVariantStatusModal" tabindex="-1" aria-labelledby="changeVariantStatusModalLabel"
        aria-hidden="true">
            ...
            document.addEventListener("DOMContentLoaded", function() {
                ...
                variantCheckboxes.on('change', function() {
                    if (!$(this).prop('checked')) {
                        selectAllVariants.prop('checked',
                            false); // Nếu có checkbox nào bỏ chọn, bỏ chọn luôn chọn tất cả
                    } else if (variantCheckboxes.length === $('.variant-checkbox:checked').length) {
                        selectAllVariants.prop('checked',
                            true); // Nếu tất cả checkbox được chọn thì check select all
                    }
                    updateDeleteButton();
                });
                ...
                const gallery = @json($gallery); // Nhận dữ liệu gallery từ Blade

                console.log('Gallery data:', gallery); // Debug dữ liệu gallery
                ...
                thumbnails.forEach(thumb => {
                    thumb.addEventListener('click', () => {
                        ...
                        if (idx >= 0 && idx < gallery.length && gallery[idx]) {
                            ...
                            console.log('Invalid index or no image at index:', idx);
                        }
                    });
                });
                ...
                if (gallery.length > 0) {
                    ...
                    if (mainImage && gallery[0]) {
                        ...
                        console.log('Set initial main image to:', gallery[0]);
                    }
                } else {
                    mainImage.src = '{{ asset('backend/assets/images/placeholder.webp') }}';
                    console.log('Gallery is empty, displaying placeholder.');
                }
                // =======================
                ...
                variantStatusBadges.forEach(badge => {
                    badge.addEventListener('click', () => {
                        ...
                        variantModalText.innerHTML =
                            `Bạn có muốn đổi trạng thái biến thể <strong>${name}</strong> sang <span class="text-primary">${nextStatusText}</span>?`;
                ...
                productStatusBadges.forEach(badge => {
                    badge.addEventListener('click', () => {
                        ...
                        productModalText.innerHTML =
                            `Bạn có chắc muốn đổi trạng thái sản phẩm <strong>${name}</strong> sang <span class="text-primary">${nextStatusText}</span>?`;
