        ...
        <div class="col-lg-6">
            <div class="border rounded-3 p-3 bg-white mb-3 shadow-sm">
                ...
                <div><strong>Trạng thái:</strong>
                    ...
                    @endphp
                    <span class="badge bg-{{ $st[1] }}"><i class="fa-solid {{ $st[2] }}"></i> {{ $st[0] }}</span>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (btn) {
        ...
        <div class="col-lg-6">
                        ...
                        <div class="d-flex align-items-center">
                            ...
                            <div>
                                ...
                                @if($item->product_variant_value_name)
                                    <div class="small text-muted">Biến thể: {{ $item->product_variant_value_name }}</div>
                                @endif
                            </div>
                        </div>
                        <div class="mt-2">
                            ...
                            <span><strong>Thành tiền:</strong> <b>{{ number_format($item->total, 0, ',', '.') }} ₫</b></span>
                        </div>
                    </div>
                @endforeach
            </div>
            <div class="border rounded-3 p-3 bg-white mb-3 shadow-sm">
                ...
                <div class="d-flex justify-content-between"><span>Phí vận chuyển:</span><span>{{ number_format($order->shipping_cost, 0, ',', '.') }} ₫</span></div>
                <div class="d-flex justify-content-between"><span>Giảm giá:</span><span>-{{ number_format($order->discount_amount, 0, ',', '.') }} ₫</span></div>
                <div class="d-flex justify-content-between fw-bold mt-2"><span>Tổng cộng:</span><span class="text-success fs-5">{{ number_format($order->total_amount, 0, ',', '.') }} ₫</span></div>
            </div>
        </div>
    </div>
    ...
    <div class="text-end">
        <a href="{{ route('client.orders.index') }}" class="btn btn-outline-secondary"><i class="fa fa-arrow-left"></i> Quay lại danh sách</a>
    </div>
</div>
@endsection
