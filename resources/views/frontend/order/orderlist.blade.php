...
<style>
    ...
    .badge-confirmed { background: #e0f7fa; color: #0097a7; }
    .badge-processing { background: #fffbe6; color: #ffc107; }
    .badge-shipped, .badge-in_transit { background: #d2eafd; color: #1976d2; }
    .badge-delivered { background: #e8f5e9; color: #43a047; }
    .badge-cancelled { background: #ffebee; color: #e53935; }
    .badge-failed_delivery { background: #f3e5f5; color: #8e24aa; }
    .badge-cod { background: #eceff1; color: #37474f; }
    .badge-bank { background: #b2ebf2; color: #0097a7; }
    .orders-filter select, .orders-filter input[type="date"], .orders-filter input[type="text"] {
        min-width: 140px;
        border-radius: 10px;
        border: 1px solid #dde7ef;
    }
    .orders-filter input[type="text"]:focus, .orders-filter input[type="date"]:focus, .orders-filter select:focus {
    ...
    .order-link { color: #059669 !important; font-weight: 600; text-decoration: underline dotted #87e3d8; }
    .table thead th, .table tbody td { vertical-align: middle; }
    /* Bo góc cho header từng ô (chỉ 2 đầu bảng) */
    .custom-table thead th:first-child { border-radius: 22px 0 0 0 !important; }
    .custom-table thead th:last-child { border-radius: 0 22px 0 0 !important; }
    .custom-table thead th {
        background: #e6fafd !important; /* cùng 1 màu xanh nhạt */
        color: #223345;
        border: none !important;
...
<div class="container">
    ...
    {{-- FILTER FORM --}}
    <form class="row gx-2 gy-2 align-items-center orders-filter mb-4 justify-content-center" method="GET" action="">
        <div class="col-auto">
        ...
        <div class="col-auto">
            <input type="date" name="date_from" class="form-control form-control-sm" placeholder="dd/mm/yyyy" value="{{ request('date_from') }}">
        </div>
        <div class="col-auto">
            <input type="date" name="date_to" class="form-control form-control-sm" placeholder="dd/mm/yyyy" value="{{ request('date_to') }}">
        </div>
        <div class="col-auto">
            <input type="text" name="keyword" class="form-control form-control-sm" placeholder="Mã đơn..." value="{{ request('keyword') }}">
        </div>
        <div class="col-auto">
            <button type="submit" class="btn btn-filter btn-sm px-4 ms-1"><i class="fa fa-filter me-1"></i> Lọc</button>
        </div>
    </form>
                ...
                <tr>
                    ...
                    <td>
                        @php
                            $statusMap = [
                                ...
                                'failed_delivery' => ['Giao thất bại', 'badge-failed_delivery', 'fa-triangle-exclamation'],
                            ];
                            $st = $statusMap[$order->status] ?? [$order->status, 'badge-pending', 'fa-question'];
                        @endphp
                    ...
                    <td>
                        ...
                        @elseif($order->payment_method == 'bank')
                            <span class="custom-badge badge-bank"><i class="fa-solid fa-building-columns"></i> Chuyển khoản</span>
                        @else
                            <span class="custom-badge badge-pending">{{ $order->payment_method }}</span>
                        @endif
                    </td>
                    <td>
                        <a href="{{ route('client.orders.show', $order->id) }}" class="text-decoration-none mx-1" title="Xem chi tiết">
