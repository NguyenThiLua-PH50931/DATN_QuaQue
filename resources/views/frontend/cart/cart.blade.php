...
@keyframes bgflash {
    0% { box-shadow: 0 4px 22px 0 rgba(253,195,44,0.08);}
    50% { box-shadow: 0 8px 30px 0 rgba(253,195,44,0.15);}
    100% { box-shadow: 0 2px 16px 0 rgba(253,195,44,0.05);}
}
...
@keyframes pulseBtn {
    0% { box-shadow: 0 0 0 0 #9eefff55; }
    70% { box-shadow: 0 0 0 8px #c2e9fb00; }
    100% { box-shadow: 0 0 0 0 #e3fcff22; }
}
...
.btn-danger.animated-bounce:hover, .btn-danger.animated-bounce:focus {
...
@media (max-width: 600px) {
    .pending-warning { padding: 15px 7px 10px 7px; }
    .pending-warning-detail { padding: 10px 3px; font-size:0.98em; }
    .pending-warning-buttons { flex-direction: column; gap:8px; }
}
    ...
    @foreach ($pendingPayments as $pendingPayment)
        ...
        <div class="pending-warning" x-data="{ showDetail: false }">
            <div class="pending-warning-content">
                ...
                <div>
                    ...
                    <div class="mt-1 text-danger">
                        👉 Nhấn <u @click="showDetail = !showDetail" style="cursor:pointer;" x-text="showDetail ? 'Đóng chi tiết' : 'Xem chi tiết'"></u> để xem lại đơn và <u>Đặt hàng ngay</u> để hoàn tất!
                    </div>
                </div>
            </div>
            ...
            <div class="pending-warning-buttons">
                <button type="button"
                    class="btn btn-outline-info animated-pulse"
            ...
            <div class="pending-warning-detail" x-show="showDetail" x-transition @click.away="showDetail = false">
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    ...
    if (sessionStorage.getItem('orderPlaced') === 'true') {
      const pendingAlert = document.querySelector('.alert-info.d-flex.justify-content-between.align-items-center');
      if (pendingAlert) {
      ...
      sessionStorage.removeItem('orderPlaced');
            ...
            document.getElementById('confirm-delete-btn').addEventListener('click', function () {
                ...
                .then(data => {
                    ...
                    if (data.success) {
                        const row = document.querySelector(`.btn-delete-item[data-id="${cartItemIdToDelete}"]`).closest('tr');
                        if (row) row.remove();
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    checkoutForm.addEventListener('submit', function(e) {
        ...
        fetch('{{ route('client.cart.checkStock') }}', {
            ...
            body: JSON.stringify({ selected_items: ids }),
        })
        .then(res => {
        ...
        .then(data => {
            if (data.success) {
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Thiếu hàng!',
                    html: (data.messages && data.messages.length) ? data.messages.join('<br>') : 'Không xác định lỗi!'
                });
            }
        })
        .catch(error => {
