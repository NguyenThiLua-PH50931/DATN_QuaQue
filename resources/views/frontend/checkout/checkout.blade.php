...
.custom-modal {
  ...
  padding: 28px 22px 18px 22px; text-align: center;
  animation: modalshow .18s cubic-bezier(.29,.86,.57,1.01);
}
@keyframes modalshow {from{opacity:0; transform:scale(.91);}to{opacity:1;transform:scale(1);}}
.custom-modal-icon {margin-bottom: 8px;}
.custom-modal-title {font-size: 1.18rem; font-weight: 600; color: #F6A721; margin-bottom: 6px;}
.custom-modal-message {color: #222; margin-bottom: 22px; font-size: 1.03rem;}
.custom-modal-btn {
  border: none; border-radius: 7px; padding: 8px 28px; font-size: 1rem;
  font-weight: 600; cursor: pointer; transition: all .14s; min-width: 95px;
  background: #16a085; color: #fff; margin-top: 3px;
}
.custom-modal-btn:hover {filter: brightness(.94);}
.custom-modal-cancel {background: #eee !important; color: #666 !important;}
                                                ...
                                                <div class="accordion accordion-flush custom-accordion" id="accordionFlushExample">
                                                    ...
                                                    <div id="flush-collapseMomo" class="accordion-collapse collapse"
                                                        ...
                                                        <div class="accordion-body text-center">
                                                            ...
                                                            @if(empty($momoResult) || (isset($momoResult['resultCode']) && $momoResult['resultCode'] != 0))
                                                                <button type="button" class="btn btn-primary mt-2" id="btn-momo-pay"></button>
                                                            @endif
                                                        </div>
                                                    </div>
                                                    ...
                                                    @if(!empty($momoResult))
                                                        <div class="alert {{ $momoResult['resultCode'] == 0 ? 'alert-success' : 'alert-danger' }}">
                                                            @if($momoResult['resultCode'] == 0)
                                                                ✅ Thanh toán MoMo thành công! Bạn hãy nhấn "Đặt hàng" để hoàn tất đơn.
                                                            @else
    ...
    <input type="hidden" name="momo_order_id" value="{{ session('pending_momo_payment.orderId') }}">
@endif
  ...
  <div class="custom-modal">
    <div class="custom-modal-icon">
      <svg width="84" height="84" viewBox="0 0 84 84" fill="none">
        ...
        <text x="50%" y="57%" text-anchor="middle" font-size="50" fill="#f8bb86" font-family="Arial" dy=".3em">!</text>
      </svg>
    </div>
    ...
    <button class="custom-modal-btn custom-modal-cancel" style="background:#eee;color:#555;margin-left:10px;">Huỷ</button>
  </div>
</div>
    ...
    if (btnOk) {
        btnOk.onclick = function () {
            ...
            fetch('/client/pay/momo', {
                ...
                body: JSON.stringify({ amount, orderId, selected_cart_item_ids: selectedCartItemIds })
            })
            .then(res => res.json())
            .then(data => {
                if (data.payUrl) {
