...
<script>
   ...
   document.addEventListener('DOMContentLoaded', function() {
    ...
    // Hàm update trạng thái enable/disable cho tất cả attribute-select
    function updateDisabledAttributes(selected, attributesCount) {
        document.querySelectorAll('.attribute-select').forEach(function(btn) {
            let attr = btn.getAttribute('data-attr');
            let val = btn.getAttribute('data-value');
            let tempSelected = Object.assign({}, selected);
            tempSelected[attr] = Number(val);

            // Loại bỏ các key null/undefined
            Object.keys(tempSelected).forEach(k => {
                if (!tempSelected[k] && tempSelected[k] !== 0) delete tempSelected[k];
            });

            // Tìm variant còn hàng phù hợp với lựa chọn tempSelected
            let variants = window.VARIANTS.filter(v => {
                let ok = true;
                for (let k in tempSelected) {
                    if (!v.value_ids.includes(Number(tempSelected[k]))) {
                        ok = false;
                        break;
                    }
                }
                return ok && Number(v.active) === 1 && Number(v.stock) > 0;
            });
    ...
    document.querySelectorAll('.attribute-select').forEach(function(btn) {
        btn.addEventListener('click', function(e) {
            ...
            updateVariantInput();
            updateDisabledAttributes(selected, attributesCount); // <- Update trạng thái ngay
            ...
            if (Object.keys(selected).length === attributesCount) {
                ...
                if (found) {
                    ...
                    // Xử lý nút thêm giỏ hàng khi hết hàng
                    const addBtn = document.querySelector('.cart-button');
                    if (addBtn) {
                        if (Number(found.stock) === 0) {
                    ...
                    updateStockOfInput({stock: 999999});
                    updateQuantityControls?.(0);
                    switchGalleryByUrl(window.DEFAULT_IMG);

                    // Cấm nút thêm giỏ hàng
                    const addBtn = document.querySelector('.cart-button');
                    if (addBtn) {
                        addBtn.setAttribute('disabled', 'disabled');
                        addBtn.classList.add('disabled');
    ...
    // Khởi tạo input ẩn và trạng thái nút khi load trang
    updateVariantInput();
    updateDisabledAttributes(selected, attributesCount);
});
