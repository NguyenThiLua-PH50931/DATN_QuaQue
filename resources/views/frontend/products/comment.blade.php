            ...
            <ul id="comments-list">
                ...
                <li data-comment-id="{{ $comment->id }}" style="list-style: none; padding-bottom: 5px;">
                    <div class="user-box border-color">
                        ...
                        {{-- Nút Sửa / Xóa / Trả lời cho comment --}}
                        <div class="comment-actions" style="display:flex; gap:10px; align-items:center;">
{{--                             @if($currentUser && $currentUser->id === $comment->user_id)
                            <button type="button" class="btn btn-sm btn-edit-comment"
                                onclick="showEditComment({{ $comment->id }}, {{ $comment->content }})">
                            ...
                            <button type="button" class="btn btn-sm" onclick="toggleReplyFormComment({{ $comment->id }})">
                    ...
                    <form class="reply-form mb-3" id="reply-form-comment-{{ $comment->id }}" data-comment-id="{{ $comment->id }}" style="display:none; margin-left: 40px;">
                        ...
                        <div style="display: flex;">
                            ...
                            <button type="button" class="btn btn-sm btn-secondary" onclick="toggleReplyFormComment({{ $comment->id }})">Hủy</button>
                        </div>
                    </form>
                    ...
                    <ul class="replies-list" style="margin-left: 40px;">
                        ...
                        <li data-reply-id="{{ $reply->id }}" style="list-style: none; padding-bottom: 5px;">
                            <div class="user-box">
                                ...
                                {{-- Nút Sửa / Xóa / Trả lời cho reply --}}
                                <div class="comment-actions" style="display:flex; gap:10px; align-items:center;">
{{--                                     @if($currentUser && $currentUser->id === $reply->user_id)
                                    <button type="button" class="btn btn-sm btn-edit-comment"
                                        onclick="showEditReply({{ $reply->id }}, $reply->reply)">
                                        <i class="fa-solid fa-pen"></i>
                                    </button>
                                    @endif --}}
                                    ...
                                    <button type="button" class="btn btn-sm" onclick="toggleReplyFormReply({{ $reply->id }})">
                                        <i class="fa-solid fa-reply"></i>
                                    </button>
                                </div>
                            </div>
                            <form class="reply-form mb-3" id="reply-form-reply-{{ $reply->id }}" data-reply-id="{{ $reply->id }}" style="display:none; margin-left: 40px;">
                                ...
                                <div style="display: flex;">
                                    ...
                                    <button type="button" class="btn btn-sm btn-secondary" onclick="toggleReplyFormReply({{ $reply->id }})">Hủy</button>
                                </div>
                            </form>
                        </li>
                        @endforeach
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Toggle form trả lời comment
    window.toggleReplyFormComment = function(commentId) {
        const form = document.getElementById(`reply-form-comment-${commentId}`);
        if (!form) return;
        form.style.display = (form.style.display === 'block') ? 'none' : 'block';
    };

    // Toggle form trả lời reply
    window.toggleReplyFormReply = function(replyId) {
    ...
    window.submitReplyComment = function(commentId) {
        ...
        .then(data => {
            if (data.status === 'success') {
                ...
                toggleReplyFormComment(commentId);
    ...
    function appendComment(comment) {
        ...
        // Chỉ có xóa/trả lời, không có nút sửa!
        const html = `
        <li data-comment-id="${comment.id}" style="list-style: none; padding-bottom: 5px;">
            <div class="user-box border-color">
