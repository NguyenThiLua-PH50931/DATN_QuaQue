...
class ReportController extends Controller
{
    ...
    public function dashboard(Request $request)
    {
        ...
        if ($cancelledType === 'week') {
            $cancelFrom = now()->startOfWeek();
            $cancelTo = now()->endOfWeek();
        } elseif ($cancelledType === 'month') {
            $cancelFrom = now()->startOfMonth();
            $cancelTo = now()->endOfMonth();
        } elseif ($cancelledType === 'year') {
            $cancelFrom = now()->startOfYear();
            $cancelTo = now()->endOfYear();
        }
        ...
        return view('backend.reports.dashboard', compact(
            ...
            'topCustomerType',
            'processingOrders',
            'processingOrdersType',
            'topCategory',
            'topCategoryType',
            'topSearchedProduct',
            'topSearchedProductType'
        ));
    }
